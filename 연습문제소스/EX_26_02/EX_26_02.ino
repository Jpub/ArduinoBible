...
void loop() {
  ...
  if (compare_it) {     // 엔터키를 누른 경우
    ...
    boolean time_pass = (interval <= MS_PER_CH * test_string_len);
    boolean typein_pass = input_string.equals(test_string);

    if (time_pass) { // 시간 내 입력
      if (typein_pass) { // 입력 내용 맞음
        Serial.println("\t** 시간 내에 정확히 입력했습니다.");
      }
      else { // 입력 내용 틀림
        Serial.println("\t** 시간 내에 입력했지만 잘못 입력했습니다.");
      }
    }
    else { // 시간 초과
      if (typein_pass) { // 입력 내용 맞음
        Serial.println("\t** 시간은 초과했지만 정확히 입력했습니다.");
      }
      else { // 입력 내용 틀림
        Serial.println("\t** 시간을 초과했고 잘못 입력했습니다.");
      }
    }
    Serial.println();
